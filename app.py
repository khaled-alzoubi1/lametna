...
def index():
    ...
    events = Event.query.order_by(Event.date.desc()).limit(4).all()
    ...
    return render_template('index.html', leaders=leaders, events=events, gallery_items=gallery_items, stats=stats)
...
def register():
    if request.method == 'POST':
        ...
        if Volunteer.query.filter_by(email=email).first():
            flash('البريد الإلكتروني مسجل مسبقاً في النظام.', 'danger')
        ...
        db.session.commit()
        flash('تم إرسال طلب انضمامك بنجاح! سيتم مراجعته واعتماده.', 'success')
...
def login():
    if request.method == 'POST':
        ...
        if identifier == 'admin' and password == 'lametna2026':
            ...
            flash('تم تسجيل الدخول كمسؤول للنظام.', 'success')
        ...
        if volunteer and check_password_hash(volunteer.password_hash, password):
            ...
            flash(f'مرحباً بك مجدداً يا {volunteer.name}', 'success')
            return redirect(url_for('profile'))

        flash('بيانات الدخول غير صحيحة، يرجى التأكد وإعادة المحاولة.', 'danger')
...
def contact_submit():
    flash('تم استلام رسالتك بنجاح وسنقوم بالرد قريباً.', 'success')
    return redirect(url_for('index'))
...
def delete_own_account():
    ...
    db.session.commit()
    session.clear()
    flash('تم حذف حسابك وجميع بياناتك نهائياً.', 'info')
    return redirect(url_for('index'))
...
def submit_excuse():
    ...
    db.session.commit()
    flash('تم إرسال العذر للإدارة بنجاح.', 'success')
    return redirect(url_for('profile'))

# ==================== لوحة تحكم الإدارة (العمليات الأساسية) ====================
...
def admin_dashboard():
    ...
    return render_template('admin.html', volunteers=volunteers, events=events)
...
def assign_leader():
    if not session.get('admin_logged_in'): return redirect(url_for('index'))
    v = Volunteer.query.get_or_404(request.form.get('volunteer_id', type=int))
    v.is_leader = True
    v.position = request.form.get('position')
    db.session.commit()
    return redirect(url_for('admin_dashboard'))
...
def adjust_events(volunteer_id, action):
    if not session.get('admin_logged_in'): return redirect(url_for('index'))
    v = Volunteer.query.get_or_404(volunteer_id)
    if action == 'increment': v.attended_events_count += 1
    elif action == 'decrement' and v.attended_events_count > 0: v.attended_events_count -= 1
    db.session.commit()
    return redirect(url_for('admin_dashboard'))
...
def adjust_hours(volunteer_id, action):
    if not session.get('admin_logged_in'): return redirect(url_for('index'))
    v = Volunteer.query.get_or_404(volunteer_id)
    if action == 'increment': v.volunteer_hours += 2
    elif action == 'decrement' and v.volunteer_hours >= 2: v.volunteer_hours -= 2
    db.session.commit()
    return redirect(url_for('admin_dashboard'))
...
# ==================== إدارة المحتوى المتقدمة للإدارة ====================
...
def assign_duty():
    ...
    db.session.commit()
    flash('تم تكليف المتطوع بالمهمة الميدانية بنجاح.', 'success')
    return redirect(url_for('admin_dashboard'))
...
def add_gallery_item():
    ...
    db.session.commit()
    flash('تمت إضافة الوسائط للمعرض بنجاح.', 'success')
    return redirect(url_for('admin_dashboard'))
